...
std::expected<void, std::string> AutoExposureService::enable(Camera& camera, const CreateAutoExposureDTO& dto) const
{
    ...
    const auto reduction = fboService.create(
    ...
    const auto chain =
        fboService.getAttachmentsOfType(memoryStorageService.frameBuffers.get(reduction.value()), FboAttachmentType::Color);
    if (chain.empty())
    {
...
void AutoExposureService::update(Camera& camera, const unsigned int ticks, const float tickSeconds) const
{
    ...
    meter.adaptedLuminance = adaptLuminance(meter.adaptedLuminance, meter.measuredLuminance, speed,
                                            static_cast<float>(ticks) * tickSeconds);
