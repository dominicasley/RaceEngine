...
[[nodiscard]] std::string telemetryToCsv(const std::vector<TelemetryFrame>& frames)
{
    ...
    for (const auto& frame : frames)
    {
        ...
        for (const auto& wheel : frame.wheels)
        {
            ...
            text += ",";
            appendNumber(text, wheel.complianceRecession * 1000.0, 3);
            text += ",";
            ...
            text += ",";
            appendNumber(text, wheel.tyreGasTemperature, 2);
            text += ",";
            appendNumber(text, wheel.tyrePressurePsi, 3);
            text += ",";
